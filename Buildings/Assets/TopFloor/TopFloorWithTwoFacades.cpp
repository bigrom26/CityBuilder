...
TopFloorWithTwoFacades::TopFloorWithTwoFacades()
{
  style = CLASSIC; // a titre d'exemple
}

void TopFloorWithTwoFacades::setBuiltIn()
{
  ...
  d3->set(depth-0.4, 0.2);
  d3->rotateOf(0, 0, 90);
  d3->translateOf(width-0.2, 0.2, height/3);
  addAsset(d3);
  
  Decoration* d4 = dynamic_cast<Decoration*>(archi->get("G_DECORATION"));
  d4->set(depth-0.4, 0.2);
  d4->rotateOf(0, 0, 90);
  d4->translateOf(width-0.2, 0.2, height*2/3);
  addAsset(d4);
